//
//  ColorConstants.swift
//  StreetLifter
//
//  Created by Artem on 05.12.2023.
//
import SwiftUI

  struct ColorConstants {
    static let bitterSweet = Color(red: 255 / 255, green: 94 / 255, blue: 91 / 255, opacity: 0.8)
    static let robinEggBlue = Color(red: 0 / 255, green: 206 / 255, blue: 203 / 255, opacity: 0.8)
    static let buttonColor = Color(red: 79 / 255, green: 93 / 255, blue: 117 / 255, opacity: 1)
      static let  chartOrange = Color(red: 255 / 255, green: 72 / 255, blue: 2 / 255, opacity: 1)
}
